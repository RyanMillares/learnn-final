...
export default function MessageBody({message}) {
    ...
    return (
        <div className = "messageBody">
            <h1 className = "responsive_text" style = {{fontWeight: 'bold'}}>{message.header}</h1>
            <a className = "responsive_text4" style = {{float: 'right'}}>Sent:<br/> {String(new Date(new Date(message.date_sent).getTime())).slice(4, 15)} {convertedTime(String(new Date(new Date(message.date_sent).getTime())).slice(16, 21)).convTime} {convertedTime(String(new Date(new Date(message.date_sent).getTime())).slice(16, 21)).isPM ? "PM" : "AM"}</a>

            <a className = "responsive_text4">From: </a> <NamePicDel memberEmail = {message.sender} isInvite = {false} key = {message.sender}/>
            <p className = "responsive_text4" style = {{marginTop: '30px'}}>{message.isInvite ? "\"" : ""}{message.content}{message.isInvite ? "\"" : ""}</p><br/>
            {
                message.isInvite && (
                    <>
                        <h1 className = "responsive_text4">You have been officially invited to join the group: <a style = {{fontWeight: 'bold'}}>{message.header.slice(message.header.indexOf(":") + 1)}</a>. To accept this invite, simply click the button below. You will return to your groups page and should be able to see the new group in your list.</h1><br/>
                        {
                            groupInfo != null && (
                                <button type = "button" className = {message.hasDecided ? "bg-gray-600 text-gray-300 rounded px-3 py-1 responsive_text4 " : "bg-green-600 text-white rounded px-3 py-1 hover:bg-green-500 responsive_text4"} onClick = {() => {
